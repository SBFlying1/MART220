...
var foodImages = [];
var foods = 3;
let foodFolder = "images/";
var foodX = [25, 25, 25];
var foodY = [200, 250, 300];
var foodLength = [50, 50, 50];
var foodWidth = [50, 50, 50];
...
function preload() {
  ...
  //Loading the Foods
  for (i = 0; i < foods; i++) {
    foodImages[i] = loadImage(foodFolder + "food" + i + ".png");
  }
...
function draw() {
  ...
  for(i = 0; i < 2; i++){
    image(foodImages[i], foodX[i], foodY[i], foodLength[i], foodWidth[i]);
  }
  ...
  if(timerValue == 0){
    timerCount++;
    if(timerCount % 4 == 0){
      image(foodImages[2], foodX[2], foodY[2], 50, 50);
    }
    else if(timerCount % 4 == 1){
      image(foodImages[2], foodY[2], foodX[2], 50, 50);
    }
    else if(timerCount % 4 == 2){
      image(foodImages[2], 525, foodY[2], 50, 50);
    }
    else if(timerCount % 4 == 3){
      image(foodImages[2], foodY[2], 525, 50, 50);
    }
    timerValue = 3;
  }
